...
func (wsc *WebSocketController) HandleConnection(c *websocket.Conn) {
	...
	if err := wsc.sendInitialState(c, gameID); err != nil {
	...
	for {
		...
		if messageType == websocket.TextMessage {
			...
			if err := wsc.handleMessage(c, gameID, playerID, msg); err != nil {
...
func (wsc *WebSocketController) handleMessage(c *websocket.Conn, gameID, playerID string, msg ws.Message) error {
...
// Helper method to send the initial game state
func (wsc *WebSocketController) sendInitialState(c *websocket.Conn, gameID string) error {
	gameState, err := wsc.gameService.GetGameState(gameID)
	if err != nil {
		return err
	}
